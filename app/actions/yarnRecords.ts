 ...
 */
const YARN_FLOAT_COLS = new Set<string>([
  "denier",
  "widthMm",
  "tensile",
  "elongation",
...
export async function fetchYarnById(input: unknown) {
  ...
  const row = await prisma.yarn.findUnique({ where: { id: idNum } });
  if (!row) return { error: "Record not found." };

  return { data: row };
}

export async function updateYarn(input: unknown) {
  ...
  for (const col of YARN_FLOAT_COLS) {
    if (col in cleaned) {
      const v = cleaned[col];
  ...
  if (shouldRecalcTenacity) {
    // We need the latest tensile/denier values:
    // - if not included in update payload, fetch from DB once
    ...
    if (!("tensile" in cleaned) || !("denier" in cleaned)) {
      const current = await prisma.yarn.findUnique({
        where: { id: idNum },
  ...
  try {
    const updated = await prisma.yarn.update({
      where: { id: idNum },
      data: cleaned,
    });
    return { data: updated };
  } catch (e) {
    console.error("Yarn update failed", e);
    return { error: "Update failed. Please check that numeric fields contain valid values." };
  }
}

export async function deleteYarn(input: unknown) {
  ...
  try {
    await prisma.yarn.delete({ where: { id: idNum } });
    return { ok: true };
  } catch (e) {
    console.error("Yarn delete failed", e);
